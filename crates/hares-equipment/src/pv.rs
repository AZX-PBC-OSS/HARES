...
impl ModuleType {
    ...
    fn from_config(config: &EquipmentConfig, key: &str) -> Self {
        config
            .get_str(key)
            .map(Self::from_str)
    ...
    fn gamma_per_c(self) -> f64 {
        match self {
            Self::Standard => DEFAULT_GAMMA_PER_C,
            Self::Premium => -0.0035,
...
impl PvArray {
    fn from_single_config(config: &EquipmentConfig) -> Result<Self, HaresError> {
    ...
    fn from_indexed_config(config: &EquipmentConfig, idx: usize) -> Result<Self, HaresError> {
...
impl PvLut {
    ...
    fn interpolate(&self, month: f64, hour: f64, ghi: f64, dni: f64, dhi: f64, temp_c: f64) -> f64 {
...
fn parse_u32_from_f64(v: Option<f64>) -> Option<u32> {
    let raw = v?;
...
fn bracket_corners(bracket: (usize, usize, f64)) -> [(usize, f64); 2] {
    let (lo, hi, t) = bracket;
    if lo == hi {
        [(lo, 1.0), (hi, 0.0)]
    } else {
...
mod tests {
    ...
    use super::{
        ...
        PvLut, cell_temperature_noct_wind, register_with_registry, surface_id_for_orientation,
    };
