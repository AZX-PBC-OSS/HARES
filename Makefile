.PHONY: check check-no-magic-config check-si-guard check-ocv check-ashrae-reference
...
check: check-no-magic-config check-si-guard check-ocv check-ashrae-reference
...
# Verify that imperial-unit markers have not spread into equipment simulation source.
check-si-guard:
	bash scripts/check-si-guard.sh
